...
class MidAirLoader(Dataset):
    def __init__(self, root_dir, mode, transform=None):
        left_dir = os.path.join(root_dir, 'image_left/')                   # left camera
        self.left_paths = sorted([os.path.join(left_dir, fname) for fname\
                           in os.listdir(left_dir)])
        
        print('Found left images in', left_dir)
        print('Found images in', left_dir, ':', self.left_paths)

        if mode == 'train':
            right_dir = os.path.join(root_dir, 'image_right/')             # right camera
            self.right_paths = sorted([os.path.join(right_dir, fname) for fname\
                                in os.listdir(right_dir)])
            
            print('Found right images in', right_dir)
