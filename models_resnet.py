...
class Monodepth(nn.Module):
    def __init__(self, num_in_layers):
        ...
        self.conv2 = conv(32, 64, 5, 2)  # H/4  -  128D
        self.conv3 = conv(64, 128, 3, 2)  # H/8  -  256D
        self.conv4 = conv(128, 256, 3, 2)  # H/16 -  512D
        self.conv5 = conv(256, 512, 3, 2)  # H/32 -  512D
        self.conv6 = conv(512, 512, 3, 2)  # H/64 -  512D
        self.conv7 = conv(512, 512, 3, 2)  # H/128 -  512D

        # decoder
        self.upconv7 = upconv(512, 512, 3, 2)
        self.iconv7 = conv(512 + 512, 512, 3, 1)
    ...
    def forward(self, x):
        ...
        x5 = self.conv5(x4)  # H/32
        x6 = self.conv6(x5)  # H/64
        x7 = self.conv7(x6)  # H/128
        
        # decoder
        upconv7 = self.upconv7(x7)
        concat7 = torch.cat((upconv7, x6), 1)
        iconv7 = self.iconv7(concat7)

        upconv6 = self.upconv6(iconv7)
        concat6 = torch.cat((upconv6, x5), 1)
        iconv6 = self.iconv6(concat6)
        ...
        upconv3 = self.upconv3(iconv4)
        concat3 = torch.cat((upconv3, x2, self.udisp4), 1)
        iconv3 = self.iconv3(concat3)
        self.disp3 = self.disp3_layer(iconv3)
        self.udisp3 = nn.functional.interpolate(self.disp3, scale_factor=2, mode='bilinear', align_corners=True)

        upconv2 = self.upconv2(iconv3)
        concat2 = torch.cat((upconv2, x1, self.udisp3), 1)
        iconv2 = self.iconv2(concat2)
        self.disp2 = self.disp2_layer(iconv2)
        self.udisp2 = nn.functional.interpolate(self.disp2, scale_factor=2, mode='bilinear', align_corners=True)
