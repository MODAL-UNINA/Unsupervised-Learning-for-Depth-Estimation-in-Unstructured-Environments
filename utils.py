...
from models.models_resnet import Resnet18_md, Resnet50_md, Monodepth
from models.models_darknet import Darknet_MidAir_attention_md
from data_loader import MidAirLoader
from models.transforms import image_transforms
...
def get_model(model, input_channels=3, pretrained=False):
    ...
    elif model == 'darknet_midair_attention_md':            # our model
        out_model = Darknet_MidAir_attention_md()
    elif model == 'monodepth':
        out_model = Monodepth(input_channels)
    else:
...
def prepare_dataloader(data_directory, mode, augment_parameters,
    ...
    data_transform = image_transforms(
        ...
        size = size)
    datasets = [MidAirLoader(os.path.join(data_directory,
